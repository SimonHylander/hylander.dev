...
export default function BragdocList({ bragdoc }: { bragdoc: BragDoc }) {
  ...
  return (
    <>
      <motion.section ref={containerRef} className={`relative z-0 mx-auto flex flex-col gap-4`}>
        <div className="flex flex-col">
          <div className="flex flex-col items-center justify-center py-8">
            <h2 className="text-2xl font-bold">Brag Document</h2>
          </div>
...
function BragdocItem({ achievement, index, scrollProgress }: { achievement: Achivement; index: number; scrollProgress: MotionValue<number> }) {
  ...
  return (
    <motion.div className={cn("w-full flex", isEven ? "justify-start" : "justify-end")}>
      <div className={cn("border-none w-full flex flex-col", isEven ? "justify-start" : "justify-end")}>
        <div className={cn(`group flex w-full justify-start items-start py-24 px-16 hover:no-underline`, isEven ? "bg-purple-900 border-1 border-purple-200" : "bg-purple-200 border-purple-200")}>
          <div className={cn("flex items-center gap-4 w-full", !isEven && "justify-end")}>
            <div className={cn(`flex flex-col gap-4 text-left text-4xl transition-opacity duration-700 ease-in-out`, isEven ? "text-red-100" : "text-purple-900")}>
              <p className="flex gap-4 text-xl text-inherit">{achievement.company}</p>

              <h3 className="flex items-center gap-4 text-inherit">
                <span className="text-4xl font-bold text-inherit">{achievement.name}</span>
              </h3>
